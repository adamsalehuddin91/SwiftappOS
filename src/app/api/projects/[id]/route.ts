...
/**
 * PATCH — partial update, and the only project write the agent may make.
 *
 * PUT takes the whole record and stays browser-only. The agent's field list is
 * enforced here rather than by the schema, because a browser PATCH is trusted
 * with the same route.
 *
 * `name` and `isArchived` are deliberately absent from the agent's list:
 * renaming a project rewrites how every past document refers to it, and
 * archiving is a disappearance rather than an edit.
 */
...
export async function PATCH(
  ...
  try {
    ...
    const refused = rejectDisallowedFields(request, body, AGENT_PROJECT_FIELDS);
